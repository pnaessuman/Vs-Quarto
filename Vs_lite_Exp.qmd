...
cordex_alpine <- read_csv2("Data/lfu_cordex_ensemble_monthly_alpine.csv")
cordex_beech <- read_csv2("Data/lfu_cordex_ensemble_monthly_beech.csv")
```
...
input_transient_s_rcp_85 <- cordex_alpine %>% 
  filter(year > 2030, rcp == "RCP_85") %>% 
  group_by(year, month) %>% 
...
spruce_forward <- vs_run_forward(spruce_params,
                                input_transient_s_rcp_85$temp,
                                input_transient_s_rcp_85$prec,
                                input_transient_s_rcp_85$syear,
                                input_transient_s_rcp_85$eyear,
                                .phi = 50)

ggplot(spruce_forward, aes(year, trw)) +
  geom_line()
```
