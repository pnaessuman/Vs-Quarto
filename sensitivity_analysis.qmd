...
shiny::selectInput("rcp_beech", "Szenario",
                   choices = c("RCP_85", "RCP_45", "RCP_26"))
shiny::selectInput("gcm_beech", "climate ids",
                   choices = c("ICHEC-EC-EARTH_r12i1p1","MIROC-MIROC5_r1i1p1", "MPI-M-MPI-ESM-LR_r1i1p1", "ICHEC-EC-EARTH_r1i1p1", "MOHC-HadGEM2-ES_r1i1p1"))
plotOutput("beech_sensitivity_rcp")
plotOutput("beech_sensitivity_gcm")
...
output$beech_sensitivity_rcp <- renderPlot({
  ...
  rcp_input <- cordex_beech %>% 
    filter(year > 2020, rcp == input$rcp_beech) %>% 
    group_by(year, month) %>% 
    summarise(
      tmean = mean(temp),
      prec = mean(prec)
  ...
  beech_forward_rcp <- vs_run_forward(beech_params2,
                                      rcp_input$tmean,
                                      rcp_input$prec,
                                      rcp_input$syear,
                                      rcp_input$eyear,
  ...
  names(beech_forward_rcp) <- c("year", "projection")
  ...
  beech_combined <- merge(beech_combined, beech_forward_rcp, all = TRUE)
  # print(head(beech_combined))
...
```
=======
>>>>>>> 9a662de85608b1be09f9f07561a811a07237cb02

>>>>>>> refs/remotes/origin/interactive
```{r}
#| context: server
output$beech_sensitivity_gcm <- renderPlot({
  ...
  gcm_input <- cordex_beech %>% 
    filter(year > 2020, gcm == input$gcm_beech) %>% 
    group_by(year, month) %>% 
    summarise(
      tmean = mean(temp),
      prec = mean(prec)
  ...
  beech_forward_gcm <- vs_run_forward(beech_params2,
               gcm_input$tmean,
               gcm_input$prec,
               gcm_input$syear,
               gcm_input$eyear,
  ...
  names(beech_forward_gcm) <- c("year", "projection")
  ...
  beech_combined$modelled <- scale(beech_combined$trw)
  beech_combined$trw <- NULL
  beech_combined <- merge(beech_combined, beech_forward_gcm, all = TRUE)
